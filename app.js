...
function renderDashboard() {
  return `
    <div class="view-header">
      <div class="view-title">${getGreeting()}, Fadl.</div>
...
function renderIC() {
  return `
    ...
    <div class="section-label">Committee Deck <span style="text-transform:none; font-weight:400; color:var(--text-faint);">— click a slide to preview it below</span></div>
    <div class="card">
      <div class="deck-strip">
        ${IC_SLIDES.map((s,i)=>`<div class="deck-thumb" id="deck-thumb-${i}" data-tip="Preview slide ${i+1}" onclick="previewSlide(${i})"><b>${String(i+1).padStart(2,'0')}</b>${s.name}</div>`).join("")}
      </div>
      <div class="slide-preview" id="slide-preview">
        <div class="slide-preview-inner" id="slide-preview-inner"></div>
      </div>
    </div>
  `;
}
let openSlideIndex = null;
function previewSlide(index) {
  const panel = document.getElementById("slide-preview");
  const inner = document.getElementById("slide-preview-inner");
  const thumbs = document.querySelectorAll(".deck-thumb");
  if (openSlideIndex === index) {
  ...
  const slide = IC_SLIDES[index];
  thumbs.forEach((t, i) => t.classList.toggle("active", i === index));
  inner.innerHTML = `
    <div class="slide-preview-eyebrow">SLIDE ${index + 1} OF ${IC_SLIDES.length} · AUTO-GENERATED BY LANI</div>
    <div class="slide-preview-title">${slide.name}</div>
    <div class="slide-preview-body">${slide.body}</div>
  `;
  panel.classList.add("open");
  setTimeout(() => panel.scrollIntoView({ behavior: "smooth", block: "nearest" }), 60);
}
afterRenderHooks.ic = () => { openSlideIndex = null; };
  ...
  return `
    ...
    <div class="two-col">
      ...
      <div class="card">
        ...
        <div class="check-row" data-tip="Click to preview" onclick="openDetail('Schedule K-1 — FY2024', 'Prior year K-1, filed and archived.', 'TAX DOCUMENT')"><div class="check-name" style="font-weight:500;">Schedule K-1 — FY2024</div><span class="pill gray">Archived</span></div>
        <div class="check-row" data-tip="Click to preview" onclick="openDetail('Annual Investor Tax Statement — FY2025', 'Summary statement covering all capital account activity, distributions, and allocated gains for the tax year.', 'TAX DOCUMENT')"><div class="check-name" style="font-weight:500;">Annual Investor Tax Statement — FY2025</div><span class="pill gray">Ready</span></div>
      </div>
    </div>
  `;
}
